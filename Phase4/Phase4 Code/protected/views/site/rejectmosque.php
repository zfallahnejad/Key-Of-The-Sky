		<?php
		$this->pageTitle=Yii::app()->name . ' - reject mosque';
			$this->breadcrumbs=array(
		'Rejectmosque', );
	if(isset($_GET["update"])){
		$connection=Yii::app()->db;
		$connection->active=TRUE;
		$sql="update mosqueculturalliablee set status = 0 WHERE (Id='$_GET[code]') ";
		$command=$connection->createCommand($sql);
		$command->execute();
		$connection->active=FALSE;
		$this->redirect('rejectmosque');
	} 
?>
	<script type='text/javascript'>
		$().ready(function(){$('#i')});// focus search area
	</script>
		<h2 class="header">لیست مساجد تاییدشده<span class="header-line"></span></h2>
								...
								foreach($mosques as $row)
								{
									...
									<tr id='io'>
										...
										<td>
											<div >
            									<h4 align="left">
													<a href="?update&code=<?php print $row["Id"]; ?>" onclick="javascript: return confirm('آیا مطمئن هستید؟');"  >عدم تایید</a>
												</h4>
											</div>
										</td>
									</tr>
